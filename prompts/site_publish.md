...
4. frontmatterに以下を入れる。
   ...
   - `eyecatchAlt`
   - `draft: false`
5. 本文から制作メモ、QAメモ、辛口読者レビュー、内部判定、露骨なメタ説明を外す。
6. 小中学生でも追える短文、改行、やさしい言葉になっているか確認する。
7. `site` で `npm run build` を実行する。
8. `db-templates/articles.csv` と `db-templates/publishing_queue.csv` の更新用CSV行を出す。
